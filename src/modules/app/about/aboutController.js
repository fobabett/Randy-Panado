...
module.exports = /*@ngInject*/
  function aboutController($scope, $state, $location, $anchorScroll, $rootScope, $timeout, $document, $stateParams, $window) {
    ...
    if($stateParams.scrollTo !== null) {
      $timeout(function() { 
        anchor = angular.element(document.getElementById($stateParams.scrollTo));
    ...
    $scope.scrollToSection = function(hash) {
      if(hash === 'credits') {
        anchor = document.getElementById('credits-section');
      } 
      else if(hash === 'equipment') {
        anchor = document.getElementById('equipment-section');
      }
      if($window.innerWidth <= 1020) {
